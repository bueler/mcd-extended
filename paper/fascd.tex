...
\end{align}
\pef{I don't see where in steps \emph{(i)-(iv)} the expression in the right of the angle brackets changes from $v - y^{j+1} - y$ (as in \eqref{eq:fe:downvinotional}) to $v - y^j$ (as in \eqref{eq:fe:downvicluttered}). From comparing the left hand sides, clearly $y^j$ in \eqref{eq:fe:downvicluttered} approximates $y$ in \eqref{eq:fe:downvinotional}, but from comparing the right hand sides, you want $y^j = y^{j+1} + y$. Maybe add a sentence, or another step, explaining what happened to the test expression?}
