...
\begin{example}  \label{ex:fnice} Suppose $f:\cV \to \cV'$ is linear, and suppose it is local in the sense that for any $z\in\mathcal{V}$ the value $\ip{f(\phi)}{z}$ can be computed by an integral over the support of $\phi$.  Considering \pr{cd-add} for concreteness, by linearity the VI can be rewritten
\begin{equation}
...
Inclusions \eqref{eq:fe:downwardsuminclusion} and \eqref{eq:fe:upwardsuminclusion}, which are special to multilevel schemes based on defect constraints, hold regardless of whether a CD operator $\Pi_i$ can be constructed.  Here there is, in fact, another ``basic'' iteration, an incomplete variant of \pr{cd-mult} (Section \ref{sec:cd}) that does not need the $\Pi_i$ operators.  One application of \pr{icd-tele} below assumes $w^J\in\mathcal{K}^J$, property \eqref{eq:fe:finestlevelequivalent}, and incomplete CD \eqref{eq:fe:upwardsuminclusion}.
