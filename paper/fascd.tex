...
FIXME rewrite the entire introduction
...
\begin{equation}
\|\rSS(w^{J,k})\|_2 < \id{atol} \qquad \text{or} \qquad \frac{\|\rSS(w^{J,k})\|_2}{\|\rSS(w^{J,0})\|_2} < \id{rtol}
\end{equation}
...
FIXME show results for Firedrake implementation of FASCD for the 2D classical obstacle problem (ball and spiral), 1D $p$-Laplacian obstacle problem (Example \ref{ex:plaplacian}), advection-diffusion obstacle problem (Example \ref{ex:advectiondiffusion}), and SIA obstacle problem
...
FIXME remove this appendix, after using it in the introduction and discussion etc.
