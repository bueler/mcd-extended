...
This paper is organized as follows.  In Section \ref{sec:vi} we recall the theory of coercive variational inequalities, with several example problems, and then Section \ref{sec:cd} reviews the basics of CDs, including the associated iterations, as proposed by Tai \cite{Tai2003}.  Section \ref{sec:femultilevel} sets up multilevel FE hierarchies, along with the special monotone restriction operators used here.  Section \ref{sec:cdmultilevel} presents defect constraints, leading to an apparently new understanding of telescoping sets for up-smoothing.  Section \ref{sec:vcycle} then presents the FASCD V-cycle, with Section \ref{sec:implementation} addressing implementation and smoother semantics, plus F-cycles.  Section \ref{sec:results} gives FASCD numerical results for the Section \ref{sec:vi} problems, followed by discussion and outlook in Section \ref{sec:discussion}.  An Appendix sketchs how FASCD reduces to classical FAS multigrid for PDEs.
...
Solvers implemented in Firedrake exploit parallel solvers from the PETSc library \cite{Balayetal2023}.  In fact, all of the above Examples, which were run in serial so as to fix the smoother,\footnote{The precise action of $O(m_j)$ work parallel smoothers depends on the number of processes \cite[for example]{Bueler2021}.} also run efficiently in parallel.  The next Example demonstrates the parallel weak scaling \cite{Bueler2021} of FASCD F-cycle solvers.
