...
FIXME observe by example that using $\iR$ or $\minR/\maxR$ to directly generate coarse constaint sets will fail; this explains defect constraint idea

Suppose $w^J \in \cK^J$ is an admissible finest-level iterate which approximates the solution $u^J$ of FE VI \eqref{eq:fe:vi}.  From $w^J$ we will now define CD constraint subsets using the defining inequalities \eqref{eq:fe:fineconstraintset} for $\cK^J$ and the above monotone restrictions $\minR, \maxR$.  Though the ideas are fundamentally the same as for unilateral constraints \cite{GraeserKornhuber2009}, the notation is necessarily more elaborate for box-constrained problems.
...
\end{equation}
From definition \eqref{eq:fe:fineconstraintset}, note that $\underline{\chi}^J \le 0 \le \overline{\chi}^J$ everywhere in $\overline{\Omega}$, including along the Dirichlet boundary $\partial_D\Omega$.  Also, if $\underline{\gamma}^J=-\infty$ then $\underline{\chi}^J=-\infty$ by definition, and similarly if $\overline{\gamma}^J=+\infty$ then $\overline{\chi}^J=+\infty$.  The motivation for creating $\underline{\chi}^J,\overline{\chi}^J$ is that a corrected iterate $w^J + z^J$ is admissible, $w^J + z^J \in \cK^J$, if and only if the perturbation $z^J$ is between the defect constraints, $\underline{\chi}^J \le z^J \le \overline{\chi}^J$, and $z^J=0$ on $\partial_D\Omega$.
...
\end{equation}
A unilateral $J=4$ example of LDCs $\underline{\chi}^j$ is shown in Figure \ref{fig:chiphilevels}.  Intuitively-speaking, the LDC $\underline{\chi}^j$ is the most-negative function from $\tilde{\mathcal{V}}^j$ such that if another element of $\tilde{\mathcal{V}}^j$ is above $\underline{\chi}^j$ then it is also above $\underline{\chi}^{j+1}$.  Similar comments apply to $\overline{\chi}^{j}$.
...
This construction of multilevel box constraints is designed to permit the largest admissible coarse corrections, which promotes multilevel solver efficiency.  Note that for the multilevel schemes in Sections \ref{sec:vcycle} and \ref{sec:implementation}, any system satisfying \eqref{eq:fe:chiordering} will suffice; the particular formulas in \eqref{eq:fe:chilevels} are not required.
...
finest-level defect constraints & ${\Large \strut} \underline{\chi}^J = \underline{\gamma}^J - w^J,\, \overline{\chi}^J = \overline{\gamma}^J - w^J$ \\
...
The multilevel V-cycle solver defined in the next Section uses LDCs $\underline{\phi}^j,\overline{\phi}^j$ for the downward side and $\underline{\chi}^j,\overline{\chi}^j$ for the upward side.  The corresponding \emph{downward/upward constraint sets}, respectively, are
\begin{align}
...
The existence of an entirely-different strategy for showing a full CD in the general box-constrained case is, of course, not excluded.
...
In summary, inclusions \eqref{eq:fe:downwardsuminclusion} and \eqref{eq:fe:upwardsuminclusion} suffice to show the admissibility of all iterates and corrections in the V-cycle algorithm which follows.  However, a convergence analysis, perhaps extending the arguments of \cite{Tai2003} or \cite{GraeserKornhuber2009} to VIs \eqref{eq:vi} which are not of minimization type, and/or to box constraints, would seem to require full CDs, and perhaps new ideas, and we cannot offer that analysis.
...
In this section we consider several practical concerns regarding the implementation and application of Algorithm \ref{alg:fascd}.  We address convergence criterion, smoothers and coarse solvers, and finally the F-cycle (sometimes called \emph{full multigrid cycle}) extension.

Finite-dimensional, box-constrained FE VI problem \eqref{eq:fe:vi} is equivalent to a \emph{mixed complementarity problem} (MCP) \cite{FacchineiPang2003}, a strong form whose statement may be clarifying.  Denote $x_p$ for a node of the finest-level mesh $\mathcal{T}^J$ and $\psi_p(x)$ for the corresponding hat function.  Let $\mathcal{X}^J=\big\{w\in\cV^J\,:\,w(x_p)=g_D^J(x_p) \text{ if } x_p\in \partial_D\Omega\big\}$, thus $\cK^J = \left\{\underline{\gamma}^J \le w \le \overline{\gamma}^J\right\} \cap \mathcal{X}^J$.  If $m_D$ is the number of nodes $x_p$ in the Dirichlet boundary, let us define $I_D$ as the $m_D\times m_J$ matrix which extracts Dirichlet nodal values; its transpose $I_D^\top$ injects Dirichlet boundary degrees of freedom into $(\mathcal{V}^J)'$.  Let us use the symbols $a\perp b$ to indicate that $a,b \in \RR^n$ are \emph{complementary}, that is, $a\ge 0$ and $b\ge 0$ and $ab = 0$ (entrywise).  Problem \eqref{eq:fe:vi}, expressed as VI$\left(f^J(\cdot)-\ell^J,\left[\,\underline{\gamma}^J,\overline{\gamma}^J\right] \cap \mathcal{X}^J\right)$ in the operations research literature \cite{FerrisPang1997}, is equivalent to the following MCP with additional equality constraints:
\begin{align}
...
MCP \eqref{eq:fe:mcp} says that exactly one of five conditions holds for the solution $u^J$ at $x_p \in \mathcal{T}^J$.  If $x_p \notin \partial_D\Omega$ then one of these four conditions hold:
\begin{itemize}
...
\end{itemize}
The cases might be labeled \emph{inactive}, \emph{lower active}, \emph{upper active}, \emph{pinched}, and \emph{boundary}, respectively.  Where case \emph{pinched} applies, $\ip{f^J(u^J)-\ell^J}{\psi_p}$ can have any real value.

Because the problem is inequality-constrained, repeated application of \pr{fascd-vcycle} to solve VI \eqref{eq:fe:vi} will not generally reduce the residual $f^J(w^J) - \ell^J$ to zero everywhere.  However, MCP \eqref{eq:fe:mcp} identifies candidate quantities which should go to zero, and which we can compare to a tolerance.  FIXME first, for $w^J \in \mathcal{K}^J$ we define the finest-level \emph{nodal complementarity residual} in $\RR^{m_J}$.  It is defined in the four cases listed after \ref{eq:fe:mcp}, namely inactive, lower active, upper active, and boundary:
\begin{equation}
...
FIXME Fischer-Burmeister semismooth alternative for convergence
...
FIXME smoothers including semi-smooth and active set \cite{BensonMunson2006}
...
\end{center}
\caption{Visualization of one iteration of \textsc{FASCD-VCYCLE} (Algorithm \ref{alg:fascd}) for Example FIXME, with $p=1.5$, $J=3$, and a coarsest mesh of two elements.  Solutions ($w^j$, $y^j$, $z^j$) are lines while obstacles ($\underline{\gamma}^j$, $\underline{\phi}^j$, $\underline{\chi}^j$) are strong dots.}
...
\section{From slack, to not lose} \label{app:slack}

FIXME move this from this appendix, after using it in the discussion etc.
