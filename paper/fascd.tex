...
\begin{abstract}
We present the \emph{full approximation storage constraint decomposition} (FASCD) multilevel method for solving variational inequalities. FASCD is a common extension of both the full approximation storage (FAS) multigrid technique for nonlinear partial differential equations, due to A.~Brandt, and of the constraint decomposition (CD) method introduced by X.-C.~Tai for variational inequalities arising in inequality-constrained optimization.  We extend the constraint decomposition idea by recognizing and exploiting the telescoping nature of certain function space subset decompositions arising from multilevel mesh hierarchies.  Then, by using strong smoothers with work proportional to the number of unknowns on the finite element mesh level, we demonstrate that FASCD V-cycles show excellent mesh-independent convergence rates and that FASCD F-cycles (full multigrid) are optimal solvers. The example problems include unilateral and bi-obstacle variational inequality problems with symmetric linear, nonsymmetric linear, and nonlinear differential operators; the examples demonstrate very good convergence even on problems where other multilevel VI solvers struggle.
...
The next VI example is a model for steady ice sheet geometry in a given climate. It is nonlinear and not of optimization type. Related VI problems arise for any fluid layer which is subject to processes that can remove the fluid \cite{Bueler2021conservation}.

\begin{example}  \label{ex:sia}  Let $\Omega \subset \RR^2$ be a fixed region of land, and suppose $b \in C^1(\Omega)$ is the given bedrock elevation.  Let $a \in L^\infty(\Omega)$ denote a given ``surface mass balance'' function, the annually-averaged rate of ice accumulation (as snow) minus melt and runoff.  Then the surface elevation $s\in C(\Omega)$ of a steady-state, isothermal, and non-sliding ice sheet (glacier) approximately satisfies the \emph{shallow ice approximation} (SIA) \cite{GreveBlatter2009} problem: find $s \in \mathcal{K} = \{s\ge b\}$ such that
\begin{equation}
...
\end{equation}
If the bed is flat ($b=0$) then a power transformation converts the problem into $p$-Laplacian form \eqref{eq:plaplacian} with $p=n+1$.  For general beds $b$ less is understood about this operator, but existence holds for \eqref{eq:siavi} \cite{JouvetBueler2012}.  The theory suggests that a power of the solution lives in a Sobolev space, namely $(s-b)^{2p/(p-1)} \in W^{1,p}(\Omega)$. \pef{`suggests' is quite weak; can we use a stronger word?} This theory, and also observations of ice sheets, shows that $|\grad s|$ is generally unbounded as one approaches the free boundary, the \emph{ice margin}, from the icy side (where $s>b$).
\end{example}
