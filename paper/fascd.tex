...
\end{center}
\caption{Visualization of one iteration of \textsc{FASCD-VCYCLE} (Algorithm \ref{alg:fascd}) in the case of Example FIXME with $p=1.5$ and $J=3$.  Level iterates and solutions ($w^j$, $y^j$, $z^j$) are lines while obstacles and defect constraints ($\underline{\gamma}^j$, $\underline{\phi}^j\le 0$, $\underline{\chi}^j\le 0$) are strong dots.  The coarsest mesh $\mathcal{T}^0$ has two elements.}
