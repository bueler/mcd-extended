...
FIXME examples---see Sections \ref{sec:vi} and \ref{sec:results}
...
The FASCD V-cycle and F-cycle (Algorithms \ref{alg:fascd}, \ref{alg:fascd-fcycle}) have been implemented in Python using the Firedrake library \cite{Rathgeberetal2016}.  In this Section we show numerical results corresponding to several VI examples from Section \ref{sec:vi}, with a focus on the scaling of FASCD iterations as the mesh resolution increases.

\begin{example}
Consider the 2D classical, unilateral obstacle problem, namely Example \ref{ex:plaplacian} with $p=2$ and admissible set $\mathcal{K} = \{v \ge \psi\} \subset W^{1,2}(\Omega)$.  Suppose $\Omega$ is a square.  We consider two problems with very different coincidence (active) sets, the ``ball'' problem with $\psi$ equal to an upper hemisphere \cite[Chapter 12]{Bueler2021}, and the ``spiral'' problem in which $\psi$ is constructed to produce a nontrivial coincidence set \cite[Example 7.1.1]{GraeserKornhuber2009}.  FIXME show table of results
\end{example}
...
\end{center}
\caption{Coincidence sets $u=\psi$ for the ball and spiral problems.}
