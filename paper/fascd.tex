...
Inclusions \eqref{eq:fe:downwardsuminclusion} and \eqref{eq:fe:upwardsuminclusion}, which are special to multilevel FE schemes based on defect constraints, hold regardless of whether a CD operator $\Pi_i$ can be constructed.  Here there is, in fact, another ``basic'' iteration, namely \pr{icd-tele} in Algorithm \ref{alg:basiccd-tele}, which also does not need the $\Pi_i$ operators.  It is a variant of \pr{cd-mult} (Algorithm \ref{alg:basiccd-mult}).
