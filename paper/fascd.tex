...
This paper is organized as follows.  In Section \ref{sec:vi} we recall the theory of coercive and monotone variational inequalities, with several example problems.  Section \ref{sec:cd} reviews the basics of CDs, as proposed by Tai \cite{Tai2003}, including the associated iterations.  Section \ref{sec:femultilevel} sets up multilevel FE hierarchies, and then Section \ref{sec:cdmultilevel} presents defect constraints generated via monotone restriction operators \cite{GraeserKornhuber2009}.  Here we propose an apparently new understanding of telescoping sets for up-smoothing, based on an ``incomplete'' CD.  Section \ref{sec:vcycle} then states the FASCD V-cycle, with Section \ref{sec:implementation} addressing convergence criteria, F-cycles, and our smoother choice.  Section \ref{sec:results} gives FASCD numerical results from a parallel Firedrake \cite{Rathgeberetal2016} implementation for the problems introduced in Section \ref{sec:vi}.  Discussion and outlook in Section \ref{sec:discussion}, and an Appendix on how FASCD reduces to classical FAS multigrid for PDEs, finish the work.
...
\end{equation}
This gives storage at most $16m_J$, $12m_J$, and $10m_J$ in dimensions $d=1,2,3$, respectively.  For comparison, a single-level method needs $4 m_J$ storage.\footnote{That is, for vectors $\underline{\gamma}^J,\overline{\gamma}^J,\ell^J,w^J$.}  If some of the obstacles are trivial, for instance in the unilateral case $\overline{\gamma}^J=+\infty$, storage is correspondingly reduced at all levels.


\section{Implementation} \label{sec:implementation}

In this section we consider several practical concerns when implementing and applying \pr{fascd-vcycle} Algorithm \ref{alg:fascd}.  We address convergence criteria, the F-cycle extension, and an active set Newton method used in Section \ref{sec:results} as a smoother and coarse solver.
...
While a large space of smoothers could be tested,\footnote{See comments in Section \ref{sec:discussion}.} Section \ref{sec:results} only demonstrates one powerful, $O(m_j)$ work per iteration, smoother choice, namely a fixed number of iterations of the active-set Newton method from \cite{BensonMunson2006}, as implemented in PETSc \cite{Balayetal2023}, with a fixed number of preconditioned Krylov iterations to solve the reduced-space step equations.  We will denote this truncated active-set Newton method as \emph{RS Newton}.  The coarse solver is the same, but with LU decomposition solution of the step equations.  For the symmetric Laplacian in Example \ref{ex:results:classical} we used conjugate gradients (CG) with incomplete Cholesky (IC) preconditioning, but for other operators the generalized minimum residual (GMRES) method with incomplete LU (ILU) preconditioning is used.
