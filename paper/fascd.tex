...
\pr{fascd-fcycle}(J,\ell^J,\underline{\gamma}^J,\overline{\gamma}^J)\text{:} \\+
    for $j=J$ downto $j=1$ \\+
        $\ell^{j-1}, \underline{\gamma}^{j-1}, \overline{\gamma}^{j-1} = R(\ell^j), \iR \underline{\gamma}^{j}, \iR \overline{\gamma}^{j}$ {\large\strut} \\-
    choose $w^0 \in \mathcal{K}^0$ \\
    $\text{\pr{solve}}(\ell^0,\underline{\gamma}^0,\overline{\gamma}^0;w^0)$ \\
    for $j=1$ to $j=J-1$ \\+
        $w^j = \max\left\{\underline{\gamma}^{j},\min\{\overline{\gamma}^{j}, Pw^{j-1}\}\right\} \in \mathcal{K}^j$ \\
...
\end{center}
\caption{Algorithm \ref{alg:fascd-fcycle} initially restricts the data $\ell^J,\underline{\gamma}^J,\overline{\gamma}^J$ down to level $j=0$ (gray), then prolongs and truncates (doubled; see lines 7 and 9 in Alg.~\ref{alg:fascd-fcycle}) to create the next-finer initial iterate.  See Figure \ref{fig:fascdvcycle} for other symbols.}
\label{fig:fcycle}
\end{figure}
