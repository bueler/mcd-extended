...
%\pef{I prefer the semicolon here because the second sentence is filling in the details for the first; without it, at the end of the first sentence, I'm thinking ``there's a citation missing here''.}
% Agreed
...
% Agreed.  Text compressed; the first item on the list already says this.
...
% Yes.  "can be generalized" is true, but in doing so you discover that (see Section 4) the \Pi_i operators in the general box-constrained CD must have a very different formula.  We actually address this by writing code that works at this level of generality.  At a certain point, if G&K can't make something clear and demo that it works then they did not do it!  We will see what we get from referees ...
...
% For F-cycles, maybe.  If something like this has been done before I want a referee to say "here it is ...".  I'd rather not guess what unpublished, undocumented, and closed extensions people have to their methods.  So I'm thinking just say what we did.  See previous point.
...
%\pef{I don't like the 2$\times$ apparently.}
% Me neither.  Fixed.
...
{\color{blue} ELB: Not being dumb.  I am describing an application I am headed toward.  FAScd applies to both differential \emph{and integral} operators.  (We don't try it out in the latter case, but it is a feature that it does not care about the locality of your operator, unlike monotone multigrid.)  It applies to operators where $f(e_i)$ makes no sense because $e_i\notin \mathcal{K}$ (SIA is an example IMO).  And it does FAS coarse corrections because we don't assume linearity of $f$.}
...
% Like it!  I shortened the caption by removing the parenthetical ("(in green; the fine degree of freedom at the same vertex is not shown)"); that's clear because of the coarse edges. 
...
This F-cycle approach is described in Algorithm \ref{alg:fascd-fcycle}.  Injection is used to generate the $\underline{\gamma}^j,\overline{\gamma}^j$ on each level.\footnote{Contrast the construction of LDCs (Section \ref{sec:cdmultilevel}) for V-cycles, based on an existing admissible iterate.}
\pef{Why is this fine for F-cycles, but not for V-cycles? I think we should spell it out.}
{\color{blue} ELB: This is the point of Example 5.1.}

\noindent
Regarding line \ref{line:fcyclecoarsestinitial}, one may generate an initial iterate $w^0\in\mathcal{K}^0$ by injection and truncation of a finest-level iterate, if one is available.
...
% Cause Paraview does it.  But I have edited it out of the .png.
...
{\color{blue} ELB: Yes.  I don't have such access currently.}
