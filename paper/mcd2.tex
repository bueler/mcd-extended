...
Let $\mathcal{V}^j$ be the continuous, piecewise-polynomial FE space $P_1$,\footnote{The FE method herein apparently extends to $Q_1$ spaces over quadrilateral meshes \cite{Elmanetal2014}.  However, for $k>1$ the $P_k$ space seems not to be suitable because the monotone restriction operator, \eqref{eq:fe:monotonerestriction} below, depends on element-wise monotonicity.  That is, for $P_1$ or $Q_1$ if the nodal values of an FE function (non-strictly) increase then the FE function also increases.  As shown by Figure 1.7 of \cite{Elmanetal2014}, for example, such monotonicity is not true for $P_2$.} over $\mathcal{T}^j$, with zero boundary values.  Note $\mathcal{V}^j$ is a subspace of $\mathcal{V}=W_0^{1,p}(\Omega)$ for any $p\ge 1$ \cite{Elmanetal2014}.  Our choices imply the following nesting:
\begin{equation}
...
One might assume that $f$ in \eqref{eq:vi} is defined over $\mathcal{K}^J$, i.e.~$f:\mathcal{K}^J \to \mathcal{V}'$.  However, especially for nontrivial nonlinear functionals, the finest-level, discretized FE functional $f^J:\mathcal{K}^J \to (\mathcal{V}^J)'$ is likely to only approximate $f$ because of quadrature, or other implementation details.  The functional $\ell$ in \eqref{eq:vi} is defined and continuous over $\mathcal{V}^J$, thus $\ell\in (\mathcal{V}^J)'$, and we assume for simplicity that it can be computed exactly, but we denote $\ell^J \in (\mathcal{V}^J)'$ to indicate that it must be represented in the FE sense.
...
While \eqref{eq:fe:vi} approximates \eqref{eq:vi}, even if quadrature and other implementation details are ignored it may not technically be conforming for \eqref{eq:vi}.  That is, despite the vector space inclusion $\mathcal{V}^J \subset \mathcal{V}$, we do not require $\mathcal{K}^J \subset \mathcal{K}$.  Only the fine-level FE obstacle $\gamma^J$ needs to be defined, while $\mathcal{K}$ remains abstract.  In fact, if a continuum obstacle exists at all then admissibility with respect to its FE interpolant $\gamma^J$ might not imply continuum admissiblity.  These caveats apply in geophysical situations where the obstacle is given by meshed elevation data, the continuum limit of which is undefined \cite{Bueler2016}.

The multilevel algorithm in the next section, designed to solve \eqref{eq:fe:vi}, requires transfer operators between meshes.  We use four such operators, three of which are the standard and linear maps used for nonlinear multigrid methods \cite{Trottenbergetal2001}.  Table \ref{tab:transfers} will help the reader keep track.
...
To define injection $\iR:\mathcal{V}^j\to\mathcal{V}^{j-1}$, observe that for any given values at the (interior) nodes of $\mathcal{T}^j$ there is a unique function in $\mathcal{V}^j$ with those values.  Also observe that, by our refinement construction, the nodes of $\mathcal{T}^{j-1}$ are also nodes of $\mathcal{T}^j$.  For $z\in\mathcal{V}^j$ we define the injection $\iR z$ as the element of $\mathcal{V}^{j-1}$ which has the same nodal values as $z$ at the nodes of $\mathcal{T}^{j-1}$.  Note that injection clearly preserves ordering, that is, if $w \le z$ then $\iR w \le \iR z$.

Finally, monotone restriction $\mR:\mathcal{V}^j\to\mathcal{V}^{j-1}$ is a nonlinear map which maximizes nodal values over the supports of coarse-mesh hat functions.  To make the definition precise, first denote the \emph{interior nodes} of triangulation $\mathcal{T}^j$ by $x_q^j$ for $q=0,\dots,m_j-1$, and then the corresponding hat function \cite{Elmanetal2014} by $\psi_q^j$.  Monotone restriction of $z\in \mathcal{V}^j$ is
\begin{equation}
  \mR z = \sum_{p=1}^{m_{j-1}} \max \{z_q \,:\, \psi_p^{j-1}(x_q^j) > 0\}\, \psi_p^{j-1}. \label{eq:fe:monotonerestriction}
\end{equation}
Recalling that the piecewise-linear functions in $\mathcal{V}^j$ have zero boundary values, note that if $z\le 0$ then $0\ge \mR z \ge z$ on $\Omega$.


\section{Nonlinear multilevel constraint decomposition} \label{sec:multilevel}

The starting point of a \emph{multilevel constraint decomposition} (MCD) \cite{GraeserKornhuber2009,Tai2003} method is to define constraint subsets, the sets generically denoted $\cK_i$ in sections \ref{sec:vi} and \ref{sec:convergence}, based on monotone restrictions of a defect constraint (see below).  Our multilevel V-cycle algorithm will define two types of constraint subsets in this manner, distinctly for the downward and upward parts of the cycle.  We also diverge from the MCD algorithms by Tai \cite{Tai2003} and Gr\"aser \& Kornhuber \cite[Algorithm 4.7]{GraeserKornhuber2009} by following a nonlinear \emph{full approximation scheme} (FAS) philosophy \cite{BrandtLivne2011}.  When $f$ is linear, as in the implemented examples in \cite{GraeserKornhuber2009,Tai2003}, there is no need to coarsen the solution itself, as in classical linear multigrid only the residual and correction are coarsened, but in the FAS approach here we coarsen the solution iterate using the injection restriction.
...
\end{equation}
a nonpositive function.  The motivation here is that a corrected iterate is admissible, $w^J + z^J \ge \gamma^J$, if and only if the perturbation $z^J$ exceeds the defect constraint, $z^J \ge \chi^J$.  Next, for $j=J,J-1,\dots,1$ we apply monotone restriction $\mR$ to define the \emph{level defect constraints}:
\begin{equation}
\chi^{j-1} = \mR \chi^j. \label{eq:fe:chilevels}
\end{equation}
As observed after equation \eqref{eq:fe:monotonerestriction}, $0\ge \chi^{j-1} \ge \chi^j$.  A $J=4$ example is shown in Figure \ref{fig:chiphilevels}.

We also denote the differences between the level defect constraints by
\begin{equation}
\phi^j = \chi^j - \chi^{j-1}. \label{eq:fe:philevels}
\end{equation}
(This uses the convenience definition $\chi^{-1}=0$, thus $\phi^0=\chi^0$ in particular.)  These are the gaps between the $\chi^j$ shown in Figure \ref{fig:chiphilevels}.  Note the telescoping sum $\sum_{j=0}^J \phi^j = \chi^J$ decomposition.
...
\pr{nmcd-vcycle}(\ell^J,\gamma^J,w^J)\text{:} \\+
    ...
    return $w^J+z^J$
\end{pseudo}
\caption{Nonlinear multilevel constraint decomposition V-cycle for the finest-level VI problem \eqref{eq:fe:vi}, using mesh-level discretizations $f^j$ of the original nonlinear $f$ in problem \eqref{eq:vi}.}
\label{alg:nmcd}
\end{pseudofloat}

During one application of \pr{nmcd-vcycle}, storage must be allocated for $\chi^j$, $\phi^j$, $g^j$, $y^j$, and $\ell^j$ on each level.  However, in algorithm lines 13 and 15 no allocation is strictly required because the $y^j$ space can be reused for $z^j$ as the coarser corrections are prolonged and accumulated.  Strictly-speaking there another such optimization: in line 3 $g^J$ need not be a copy of $w^J$.  The total memory usage of \pr{nmcd-vcycle}, excluding storage usage in the smoothers/solver but adding storage for the original obstacle $\gamma^J$, is about $8 m^J$ real numbers\footnote{That is: $8\approx 5(4/3) + 1$.} under the standard estimate that a 2D, uniform-mesh V-cycle uses $4/3$ the memory of the finest level alone \cite[Section 2.4]{Trottenbergetal2001}, whereas a single-level method needs only $3 m^J$ for vectors $\ell^J,w^J,\gamma^J$ themselves.

Regarding notation and semantics for the smoothers, note they may be iterated \id{down} and \id{up} times, respectively.  Both the smoothers and the coarsest-level solver are supposed to do in-place modifications of their final arguments, and of course they are in-exact solvers.  They must not modify their first three arguments.

Furthermore, repeated application of \pr{nmcd-vcycle}, or of the smoothers and solver at the levels, will not reduce the classical residual $f^J(u^J) - \ell^J$ to zero everywhere.  Recall that a finite-dimensional VI problem is equivalent to a \emph{nonlinear complementarity problem} (NCP) \cite{FacchineiPang2003}.  For example, corresponding to \eqref{eq:fe:vi} one could write
\begin{equation}
u^J \ge \gamma^J, \qquad f^J(u^J) \ge \ell^J, \qquad (u^J - \gamma^J)\left(f^J(u^J) - \ell^J\right) = 0.  \label{eq:fe:ncp}
\end{equation}
Thus, at convergence we only expect $f^J(u^J) = \ell^J$ at those locations where the constraint is inactive, where $u^J > \gamma^J$.  Therefore, recalling that a $j$th-level hat function based at node $x_p^j$ is denoted $\psi_p^j$, for an iterate $w^j$ define the \emph{downward nodal residual} by
\begin{equation}
(\hat r_{\text{down}}^j(w))_p = \begin{cases} \ip{f^j(w^j)-\ell^j}{\psi_p^j}, & w^j_p > \phi^j_p, \\
                                  \min\{\ip{f^j(w^j)-\ell^j}{\psi_p^j},0\}, & w^j_p = \phi^j_p, \end{cases} \label{eq:cpresidual}
\end{equation}
where $w^j_p,\phi^j_p$ denote the nodal values of $w^j,\phi^j$ as $x_p^j$, respectively.  The \emph{upward nodal residual} $\hat r_{\text{up}}(w)$ is similarly defined using $\chi^j$.

The nodal residuals $\hat r_{\text{down}}(w^j),\hat r_{\text{up}}(w^j)$ are nonzero if there are nodes where the NCP \eqref{eq:fe:ncp} is violated, either because $f^j(w^j)-\ell^j$ remains nonzero, in a weak sense, on the inactive set or because the same quantity is negative on the active set.  If $w^j$ is admissible, for instance if $w^j = g^j+z^j$ for $z^j \in \mathcal{U}^j$, inside \pr{nmcd-vcycle}, and if $\|\hat r_{\text{up}}(w^j)\|$ is small in some appropriate norm then we regard the $j$th-level problem as being numerically solved.  This provides stopping criteria for application of \pr{solver} on the coarsest mesh, and for terminating cycles of \pr{nmcd-vcycle} when addressing the original VI \eqref{eq:fe:vi}.

FIXME in Appendix we show how NMCD reduces to FAS if the obstacle is removed and to MCD if $f$ is assumed to be linear

FIXME again observe that up-smoothing is more efficient
