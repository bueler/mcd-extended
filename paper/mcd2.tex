...
Suppose there are $m<\infty$ subspaces $\cV_i \subset \cV$ so that the sum
\begin{equation}
...
\end{equation}
The sum in \eqref{eq:constraintdecomp} is required to hold in two senses: \emph{(i)}~if $w \in \cK$ then there exist $w_i \in \cK_i$ so that $w = \sum_i w_i$, and \emph{(ii)}~if $z_i \in \cK_i$ for each $i$ then $\sum_i z_i \in \cK$.  (The latter sense for \eqref{eq:subspacedecomp} is automatic because the $\cV_i$ are subspaces.)  Note that neither decomposition \eqref{eq:subspacedecomp} or \eqref{eq:constraintdecomp} is required to be unique in any sense.  Also, $\cK_i \not\subset \cK$ in many applications; see the cartoon in Figure \ref{fig:cartoon}.  Finally, for each $\cK_i$ we assume that there are bounded, (generally) nonlinear restriction operators $R_i : \cK \to \cK_i$ such that
\begin{equation}
...
In Section \ref{sec:multilevel} we will introduce discretizations and describe practical algorithms, but the fundamental CD concept applies even at the level of the continuum problem.  The following two examples illustrate this for obstacle problems \cite{GraeserKornhuber2009}, the first being an overlapping domain decomposition and the second a disjoint frequency decomposition.  The multilevel FE CD in Section \ref{sec:multilevel} approximates such an exact frequency decomposition.
...
The reader may confirm that, inside each boxed VI in Algorithm \ref{alg:basiccd}, the argument of $f$ is an element of $\cK$.  While the $\cK_i$ difference $v_i - \hat w_i$ appears, note that by \eqref{eq:constraintdecomp} and \eqref{eq:constraintrestrictionsum} one may write it as a difference of admissible vectors (i.e.~from $\cK$), namely
\begin{align*}
...
\end{align*}
for the two versions, respectively.  In other words, we solve VI \eqref{eq:vi} over (parallel or successive) perturbations from the sets $\cK_i$.

In fact, if we define
\begin{equation}
e_i = \hat w_i - R_i u \in \cV_i \label{eq:ithupdate}
\end{equation}
as the $i$th update from a pass through a \textbf{for} loop in Algorithm \ref{alg:basiccd} then $\hat w = u + \sum_{i} e_i$ and $w = u + \alpha \sum_i e_i$.  Easy identities $u - R_i u + \hat w_i = u + e_i$ and $\hat w = u^* + \sum_i \hat w_i - R_i u^*$ will also be used.

The convergence results in the next section require substantial damping when applying the parallel algorithm, i.e.~$\alpha \le 1/m$ \cite{Tai2003}, but the successive version can be shown to converge without damping ($\alpha=1$).  In Section \ref{sec:results} we demonstrate practical convergence for a larger range of $\alpha$ than suggested by the theory.  % HOPE

In order for Algorithm \ref{alg:basiccd} to have a practical and efficient FE implementation (Section \ref{sec:multilevel}), we need additional notions when $f$ has any of the following properties: \emph{(i)} $f$ is defined only on $\cK$, or \emph{(ii)} $f$ is nonlinear, or \emph{(iii)} $f$ is non-local.  The issue of practical implementation for any such $f$ seems not to have been addressed in the CD literature; in particular  \cite{GraeserKornhuber2009,Tai2003} only apply the method to the classical obstacle problem.  The following example explains the simplifications available in that easier case.

\begin{example}  \label{ex:fnice} Suppose $f:\cV \to \cV'$ is linear and defined on all of $\cV$.  Furthermore suppose $f$ is local in the sense that a basis $\{\phi_i\}$ of $\cV_i$ exists with each support small in $\Omega$ in the sense that $\ip{f(\phi_i)}{z}$ can be computed by an integral over the support of $\phi_i$.  (When $\cV$ is an FE space then this is the usual case for evaluating PDE weak forms over a basis of hat functions---see Section \ref{sec:multilevel}---but integral operators do not generally have this property.)  Considering only the parallel algorithm for simplicity, the VI over $\cK_i$ in Algorithm \ref{alg:basiccd} can be written as
\begin{equation}
\ip{f(\hat w_i - R_i u)}{v_i-\hat w_i} \ge \tilde g(v_i-\hat w_i) \label{eq:linearlocalvi}
\end{equation}
where $\tilde g(z) = g(z) - \ip{f(u)}{z}$.  Noting $e_i = \hat w_i - R_i u \in \cV_i \notin \cK$, in general, VI \eqref{eq:linearlocalvi} only makes sense because $f$ is defined over $\cV_i$.  In this case each subset problem \eqref{eq:linearlocalvi} can be solved using a stored residual $\ip{f(u)}{\cdot}$ included into the updated source term.  The VI \eqref{eq:vi} is then approximately solved in any incremental manner, by computations over the basis supports, by one application of \pr{cditeration}.
\end{example}

In other words, because $f$ has all these nice properties, the decomposed VI problems in Example \ref{ex:fnice} can be computed by inexpensive means.  Note that the solution method for VI \eqref{eq:linearlocalvi} is not the concern here; we are observing that the data of problem \eqref{eq:linearlocalvi} is small in practice.

A solver which would work for Example \ref{ex:fnice} can thus have implementation efficiencies which are unavailable in general.  Additional ideas, like those in Section \ref{sec:multilevel}, are therefore needed for practical application of CD methods to more difficult problems.  In particular, we will extend the basic algorithm to nonlinear $f$ by applying the full approximation storage (FAS) idea of Brandt \cite{Brandt1977}, in which case the decomposed problems only evaluate $f$ over (admissible) elements of $\mathcal{K}$.  However, extending the algorithm to non-local residual functionals is a topic for future research.


\section{Convergence of the basic algorithm} \label{sec:convergence}

To prove the convergence of the basic algorithm we make two assumptions, as follows, which are inequalities (7) and (8) in \cite{Tai2003}.  These inequalities might be described as ``totally Lipschitz'' requirements for the maps $R_i$ and $f$, respectively, over the constraint decomposition.
...
\end{equation}
for all $u,v\in\cK$.  Furthermore, there exists $C_2>0$ so that
\begin{equation}
...
\end{equation}
for all $w_{ij} \in \cK$ and $v_i,y_i \in \cV_i$ such that $w_{ij} + v_i \in \cK$ (for all $i,j$).
\end{assumptions*}

These technical assumptions have some essential features:
\begin{itemize}
\item Assumption \eqref{as:lipschitzrestrictions} addresses only the constraint decomposition, and not the map $f$.
\item It is sometimes the case that \eqref{as:lipschitzresidual} addresses only a subspace decomposition property of the operator $f$, and not the CD sets $\cK_i$ or maps $R_i$.  For example, in the classical obstacle problem \eqref{as:lipschitzresidual} is verified over all $w_{ij} \in \cV$.
...
\item Tai \cite{Tai2003} gives values of the constants $C_1$ in \eqref{as:lipschitzrestrictions} for obstacle problem constraint decompositions using $P_1$ FE spaces over shape-regular and quasi-uniform triangulations, which is the only case we will demonstrate (Section \ref{sec:results}).  In particular, constants $C_1$ are know for overlapping domain decompositions (with or without an additional coarse mesh) and standard multilevel hierarchies.
\item Constants $C_1,C_2$ generally depend on $m$.  However, as noted in \cite{Tai2003}, and regarding the convergence proof below, if repeated application of Algorithm \ref{alg:basiccd} is known to generate a bounded sequence of iterates in $\mathcal{K}$ then constants $C_1,C_2$ are actually permitted to depend on $u,v,w_{ij},v_i,y_i$ as well.
% ASSUMING $f$ IS LIPSCHITZ WILL MAKE THE EXISTENCE OF C_2 CLEAR; SEE (14) IN \cite{TaiXu2002}
\end{itemize}

From the assumptions and the above definitions we have the following core lemma for the parallel version.  Our proof follows \cite{Tai2003}.

\begin{lemma} \label{lem:core}  Suppose $u^* \in \mathcal{K}$ solves \eqref{eq:vi}.  Suppose $\mathcal{V}_i$, $\mathcal{K}_i$, $R_i$, and $f$ satisfy \eqref{as:lipschitzrestrictions} and \eqref{as:lipschitzresidual}.  Suppose $\hat w$ is computed from $u$ by the parallel version of Algorithm \ref{alg:basiccd}.  Recall $e_i = \hat w_i - R_i u$.  Then
\begin{equation}
...
\begin{proof}
For $i\in \{0,1,\dots,m-1\}$ use increasing, but wrapped, indices to define the following sequence $\phi_{ij}=\phi_{i,j} \in \cK$:
\begin{align*}
\phi_{i,0} &= u + e_i, \\
...
\end{align*}
(Each $\phi_{ij} \in \cK$ by \eqref{eq:constraintdecomp} and \eqref{eq:constraintrestrictionsum}.)  Noting that the boxed VI in the parallel version of Algorithm \ref{alg:basiccd} can be written

    FIXME
\end{proof}

Note that the coercivity of $f$ is not used in proving Lemma \ref{lem:core}.
