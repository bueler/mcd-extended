...
\end{equation}
Note that $v-u^* \in W_0^{1,p}(\Omega)$, that is, the test functions in \eqref{eq:boxdirichletvi} have zero values on the Dirichlet boundary $\partial_D\Omega$.  In general, problem \eqref{eq:boxdirichletvi} is a mixed boundary value problem with homogeneous Neumann, i.e.~natural, boundary conditions over $\partial_N \Omega = \Omega \setminus \partial_D \Omega$.
...
FIXME READ FROM HERE Finally, the monotone restrictions $\maxR,\minR$ are nonlinear maps which maximize and minimize, respectively, nodal values over the supports of coarser-mesh hat functions.  To make the definitions precise, first denote the \emph{nodes} of mesh $\mathcal{T}^j$ by $x_q^j$ for $q=0,\dots,m_j-1$, and then the corresponding hat functions \cite{Elmanetal2014} by $\psi_q^j$.  (Boundary nodes are included here.)  The monotone restrictions of $z\in\tilde{\mathcal{V}}^j$ are
\begin{align}
