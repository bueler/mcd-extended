...
The starting point of a \emph{multilevel constraint decomposition} (MCD) \cite{GraeserKornhuber2009,Tai2003} method is to define constraint subsets, the sets generically denoted $\cK_i$ in sections \ref{sec:vi} and \ref{sec:convergence}, based on monotone restrictions of the constraint.  In fact our algorithm is a multilevel V-cycle with two types of constraint subsets, distinctly for the downward and upward parts of the cycle.
...
\end{equation}
a nonpositive function.  The motivation of the defect constraint is that a corrected iterate is admissible, $w^J + z^J \ge \gamma^J$, if and only if the perturbation $z^J$ exceeds the defect constraint, $z^J \ge \chi^J$.  Next, for $j=J,J-1,\dots,1$ apply monotone restriction $\mR$ to define the \emph{level defect constraints}
\begin{equation}
...
For $j=0,\dots,J$ we also denote the differences between the level defect constraints by
\begin{equation}
\phi^j = \chi^j - \chi^{j-1}. \label{eq:fe:philevels}
\end{equation}
(This uses the convenience definition $\chi^{-1}=0$.)  These are the gaps between the $\chi^j$ shown in Figure \ref{fig:chiphilevels}.  Note $\phi^0=\chi^0$, and that the telescoping sum $\sum_{j=0}^J \phi^j = \chi^J$ applies.
...
Returning to the heart of the MCD idea, our plan is to correct the admissible fine-level iterate $w^J\in \mathcal{K}^J$ by adding modifications from each coarser subspace $\mathcal{V}^j$.  By nesting \eqref{eq:fe:nestedspaces} these corrections live in the fine-level subspace $\mathcal{V}^J$, but the corrected iterate needs to be admissible as well.  The accumulated corrections, from the constraint subsets $\mathcal{D}^j$ and $\mathcal{U}^j$, over the whole multigrid V-cycle, compute a new iterate in $\mathcal{K}^J$.

In detail, suppose $y^{j+1} \in \mathcal{D}^{j+1}, \dots, y^J \in \mathcal{D}^J$ are already-computed corrections during the downward part of the V-cycle (Figure \ref{fig:nmcdvcycle}).  The so-far corrected iterate is admissible, $w^J + y^J + \dots + y^{j+1} \in \mathcal{K}^J$, because the sum of corrections is in the (finest) defect constraint set, namely $y^J + \dots + y^{j+1} \in \mathcal{U}^J = \{v\ge \chi^J\}$.
...
Now, when computing the next-coarser correction we will need to use the coarser-mesh representation, and, as in nonlinear FAS multigrid schemes \cite{BrandtLivne2011,Bruneetal2015,Trottenbergetal2001}, we must coarsen the iterate as well as the residual and the correction.\footnote{Here we diverge from algorithms by Tai  \cite{Tai2003} and Gr\"aser \& Kornhuber \cite[Algorithm 4.7]{GraeserKornhuber2009}.  If $f$ is linear, as in the implemented examples in those works, there is no need to coarsen the solution.  (In classical linear multigrid only the residual and the correction must be coarsened.)  We follow the FAS philosophy for nonlinear functionals.}  We do this using injection \cite[section 5.3]{Trottenbergetal2001}.  Define
\begin{equation}
g^j = \begin{cases} w^J, & j=J \\
                    \iR(g^{j+1} + y^{j+1}), & j < J.
      \end{cases}  \label{eq:fe:defineg}
\end{equation}
That is, $g^j$ is a $j$th-level mesh approximation to the solution $u^J$, before we add the correction which solves the coarsened VI problem at the $j$th level.\footnote{If we were to inject the finest-mesh obstacle downward to coarser meshes, i.e.~$\gamma^{j-1} = \iR \gamma^j$, then, because $\iR$ preserves inequalities and $y^j \in \mathcal{D}^j$  it would follow that $g^j \ge \gamma^j$.  In this sense $g^j$ is an admissible function on the $j$th-level mesh.  However, our use of the defect constraints $\chi^j$, following \cite{GraeserKornhuber2009}, avoids the need to generate these $\gamma^j$; their definition in this footnote can be ignored.}

FIXME do I need $f^j$ to be defined?  yes.  Tai assumes $f(w^J+y^J + \dots + y^{j+1} +y^j)$ is defined, which is true, but $f^j(g^j)$ uses the $j$th level representation
...
\pr{nmcd-vcycle}(\ell^J,w^J,\gamma^J)\text{:} \\+
    $\chi^J = w^J - \gamma^J$ \\
    $g^J = w^J$ \ct{notational; no copy needed} \\
    for $j=J$ downto $j=1$ \\+
      $\chi^{j-1} = \mR \chi^j$ \\
      $\phi^j = \chi^j - P\chi^{j-1}$ \\
      $y^j = 0$ \\
      $\text{\pr{smoother}}^{\text{\id{down}}}(f^j,\ell^j,\phi^j,g^j;y^j)$  \ct{smoothing in $\mathcal{D}^j$}\\
      $g^{j-1} = \iR(g^j + y^j)$ \\
      $\ell^{j-1} = f^{j-1}(g^{j-1}) - R \left(f^j(g^j+y^j) - \ell^j\right)$ \\-
    $y^0 = 0$ \\
    $\text{\pr{solver}}^{\text{\id{coarse}}}(f^0,\ell^0,\chi^0,g^0;y^0)$ \ct{solving in $\mathcal{D}^0=\mathcal{U}^0$} \\
    $z^0 = y^0$ \\
    for $j=1$ to $j=J$ \\+
      $z^j = P z^{j-1} + y^{j}$ \\
      $\text{\pr{smoother}}^{\text{\id{up}}}(f^j,\ell^j,\chi^j,g^j;z^j)$  \ct{smoothing in $\mathcal{U}^j$} \\-
    return $w^J+z^J$
\end{pseudo*}
