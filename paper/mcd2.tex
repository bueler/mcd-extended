...
\end{equation}
a nonpositive function.  The idea of the defect constraint is that a perturbed iterate is admissible, e.g.~$w^J + z^J \ge \psi^J$, if and only if the perturbation $z^J$ exceeds the defect constraint, $z^J \ge \psi^j$.  Now for $j=J$ down to $j=1$ apply monotone restriction $\mR$ to define
\begin{equation}
\chi^{j-1} = \mR \chi^j, \label{eq:fe:chilevels}
\end{equation}
called the \emph{level defect constraints}.  A four-level example is shown in Figure \ref{fig:chiphilevels}.
...
Denoting $\chi^{-1}=0$, for $j=0,\dots,J$ we also define the differences
\begin{equation}
...
For the downward part of the multigrid V-cycle we will use the $\phi^j$ as constraints, and for the upward part the $\chi^j$.  That is, we define constraint subsets
\begin{equation}
\mathcal{D}^j = \{v\ge \phi^j\}, \qquad \mathcal{U}^j = \{v\ge \chi^j\}. \label{eq:fe:constraintsets}
\end{equation}
For $j=0,\dots,J$ these are subsets of the FE spaces $\mathcal{V}^j$.  The upward sets are larger, and in fact the decomposition $\mathcal{U}^j = \sum_{i=0}^j \mathcal{D}^i$ applies for each $j$ in the same sense as \eqref{eq:constraintdecomp}.

FIXME explain $g$ and $y $ and $z$ etc.; explain smoother semantics (in-place on final argument)
...
\pr{nmcd-vcycle}(f,\ell,w^J,\gamma^J)\text{:} \\+
    $\ell^J = \ell$ \ct{notational} \\
    $\chi^J = w^J - \gamma^J$ \\
    $g^J = w^J$ \ct{notational} \\
    for $j=J$ downto $j=1$ \\+
      $\chi^{j-1} = \mR \chi^j$ \\
      $\phi^j = \chi^j - P\chi^{j-1}$ \\
      $y^j = 0$ \\
      $\text{\pr{smoother}}^{\text{\id{down}}}(f,\ell^j,\phi^j,g^j;y^j)$  \ct{smoothing in $\mathcal{D}^j$}\\
      $g^{j-1} = \iR(g^j + y^j)$ \\
      $\ell^{j-1} = f(g^{j-1}) - R \left(f(g^j+y^j) - \ell^j\right)$ \\-
    $y^0 = 0$ \\
    $\text{\pr{solver}}^{\text{\id{coarse}}}(f,\ell^0,\chi^0,g^0;y^0)$ \ct{solving in $\mathcal{D}^0=\mathcal{U}^0$} \\
    $z^0 = y^0$ \\
    for $j=1$ to $j=J$ \\+
      $z^j = P z^{j-1} + y^{j}$ \\
      $\text{\pr{smoother}}^{\text{\id{up}}}(f,\ell^j,\chi^j,g^j;z^j)$  \ct{smoothing in $\mathcal{U}^j$} \\-
    return $w^J+z^J$
\end{pseudo*}
