...
To define the CD constraint subsets in the current multilevel FE context, suppose $w^J \in \cK^J$ is an admissible finest-level iterate which approximates the solution $u^J$ of VI \eqref{eq:fe:vi}.  From this iterate we define the \emph{finest-level lower/upper defect constraints}
\begin{equation}
\underline{\chi}^J = \underline{\gamma}^J - w^J, \qquad \overline{\chi}^J = \overline{\gamma}^J - w^J, \label{eq:fe:defectconstraints}
\end{equation}
as functions in $\tilde{\mathcal{V}}^J$.  Note that $\underline{\chi}^J \le 0 \le \overline{\chi}^J$ everywhere in $\overline{\Omega}$, including along the Dirichlet boundary $\partial_D\Omega$.  The motivation for such defect constraints \cite{GraeserKornhuber2009} is that a corrected iterate is admissible with respect to the defining inequalities for $\cK^J$, namely $\underline{\gamma}^J \le w^J + z^J \le \overline{\gamma}^J$, if and only if the perturbation $z^J$ is between the defect constraints, $\underline{\chi}^J \le z^J \le \overline{\chi}^J$.

For coarser levels $j=J-1,\dots,1,0$ we apply monotone restrictions to define the \emph{level defect constraints},
\begin{equation}
\underline{\chi}^{j} = \maxR \underline{\chi}^{j+1}, \qquad \overline{\chi}^{j} = \minR \overline{\chi}^{j+1}. \label{eq:fe:chilevels}
\end{equation}
FIXME: VALUES ALONG $\partial_D\Omega$ ARE BY INJECTION  Observe that
\begin{equation}
