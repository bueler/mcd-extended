...
FIXME coarse triangulation, refine to level $J$, canonical restriction $R$, canonical prolongation $P$, injection $\iR$, monotone restriction $\mR$

Suppose our fine-level FE subspace is $\mathcal{V}^J \subset \mathcal{V}$, based on $J$ levels of mesh refinement as above.  Note that the functional $g$ in \eqref{eq:vi} is defined and continuous over $\mathcal{V}^J$, that is, $g\in (\mathcal{V}^J)'$.  If $\psi^J \in \mathcal{V}^J$ denotes the fine-level \emph{obstacle}, define
\begin{equation}
\mathcal{K}^J = \left\{v \in \mathcal{V}^J\,:\,v \ge \psi^J\right\}, \label{eq:fineconstraintset}
\end{equation}
...
As a technical matter, VI \eqref{eq:fe:vi} is not necessarily conforming for \eqref{eq:vi} even though we assume $\mathcal{V}^J \subset \mathcal{V}$ at the vector space level.  Indeed, we do not specifically require $\mathcal{K}^J \subset \mathcal{K}$, and only the fine-level FE obstacle $\psi^J$ needs to be concretely defined here, while $\mathcal{K}$ remains abstract.  This situation applies in certain geophysical situations where the obstacle is given by meshed data, the continuum limit of which is unknown \cite{Bueler2016}.

The algorithm in the next section is designed to solve \eqref{eq:fe:vi}.


\section{Nonlinear multilevel constraint decomposition} \label{sec:multilevel}

The starting point of a \emph{multilevel constraint decomposition} \cite{GraeserKornhuber2009,Tai2003} method is to create constraint subsets, the sets denoted $\cK_i$ in sections \ref{sec:vi} and \ref{sec:convergence}, based on monotone restrictions of the constraint.  In fact the algorithm in the current section is a multigrid V-cycle with two systems of constraint subsets, one for the downward part of the cycle and one for the upward part.

To make this precise, suppose $w^J \in \cK^J$ is an admissible fine-level iterate which approximates the solution $u^J$ in some sense.  From the iterate we define the \emph{defect constraint} \cite{GraeserKornhuber2009}
\begin{equation}
\chi^J = \psi^J - w^J,  \label{eq:fe:defectconstraintfcn}
\end{equation}
a nonpositive function.  For $j=J$ down to $j=1$ apply monotone restriction $\mR$ to define
\begin{equation}
\chi^{j-1} = \mR \chi^j, \label{eq:fe:chilevels}
\end{equation}
the \emph{level defect constraints}.  A four-level example is shown in Figure \ref{fig:chiphilevels}.  Denoting $\chi^{-1}=0$, for $j=0,\dots,J$ we also define the differences
\begin{equation}
\phi^j = \chi^j - \chi^{j-1}. \label{eq:fe:philevels}
\end{equation}
Note $\phi^0=\chi^0$ and that a telescoping sum applies, namely $\sum_{j=0}^J \phi^j = \chi^J$.  Figure \ref{fig:chiphilevels} shows the functions $\phi^j$ as the gaps between the level defect constraints $\chi^j$.

%REGENERATE using https://github.com/bueler/mg-glaciers:
%$ cd mg-glaciers/py/
%$ [modify decomposition() inside visualize.py to print \psi]
...
\caption{Defect constraint $\chi^J = \psi^J - w^J$ is decomposed using monotone restriction.}
\label{fig:chiphilevels}
\end{figure}

FIXME
    $$\mathcal{D}^J = \{v\ge \chi^J\} \subset \mathcal{V}^J.$$
(Recall that a perturbed iterate is admissible, $w^J + z^J \ge b^J$, if and only if the perturbation $z^J$ is in $\mathcal{D}^J$.)  Let $N^J$ be an FE approximation of the nonlinear operator $N_\eps$ in \eqref{eq:siaN}, but note that the detailed construction of $N^J$ occurs in the next subsection.  Let $\ell^J[v] = \ip{a}{v}$.  Our goal is to solve the fine-level FE version of VI \eqref{eq:siaVI} for an admissible perturbation $z^J$ in $\mathcal{D}^J$,
\begin{equation}
N^J(w^J+z^J)[v-z^J] \ge \ell^J[v-z^J] \label{eq:siaVIFEfinelevel}
\end{equation}
for all $v$ in $\mathcal{D}^J$.  Here $s^J=w^J+z^J$ is the desired surface elevation result, so the argument $v - z^J = (w^J+v)-s^J$ is a difference of admissible surface elevations.
