...
A multilevel V-cycle is defined in the next Section.  For the downward side of the V-cycle we will use the LDCs $\underline{\phi}^j,\overline{\phi}^j$ as constraints, and for the upward side the LDCs $\underline{\chi}^j,\overline{\chi}^j$; see \eqref{eq:fe:philevels} and \eqref{eq:fe:chilevels}, respectively.  In the current Section we show how these level defect constraint functions generate constraint sets and constraint decompositions (CDs).
...
\end{align}
be the \emph{downward} and \emph{upward defect constraint subsets}, respectively, of the FE subspaces $\mathcal{V}^j$, for $j=0,1,\dots,J$.  Again these subsets are closed and convex.  Observe that the upward sets are larger.  By \eqref{eq:fe:telescoping}, we have $\mathcal{D}^j \subset \mathcal{U}^j$ and set decompositions
\begin{equation}
\mathcal{U}^j = \sum_{i=0}^j \mathcal{D}^i \label{eq:fe:constraintdecomp}
\end{equation}
in the sense of \eqref{eq:constraintdecomp}.  Also note that the finest-level upward constraint set $\mathcal{U}^J$ is equivalent to the original constraint set $\mathcal{K}^J$: for all $z^J \in \mathcal{V}^J$,
\begin{equation}
z^J \in \mathcal{U}^J \quad \iff \quad w^J+z^J \in \mathcal{K}^J. \label{eq:fe:finestlevelequivalent}
\end{equation}

Recalling definitions \eqref{eq:subspacedecomp}--\eqref{eq:constraintrestrictionsum}, we now have two varieties of multilevel CDs of each upward defect constraint set $\mathcal{U}^j$.

\begin{lemma}  For any $j=0,1,\dots,J$ and $0\le k\le j$ the decomposition sums
\begin{align}
\mathcal{U}^j &= \mathcal{D}^j + \mathcal{D}^{j-1} + \dots + \mathcal{D}^0, \label{eq:fe:downwardsumcd} \\
\mathcal{U}^j &= \mathcal{D}^j + \mathcal{D}^{j-1} + \dots + \mathcal{D}^{k+1} + \mathcal{U}^k \label{eq:fe:upwardsumcd}
\end{align}
are CDs of\, $\mathcal{U}^j$ over subspace decompositions $\cV^j=\sum_{i=0}^j \cV^i$ and $\cV^j=\sum_{i=k}^j \cV^i$, respectively.
\end{lemma}

\begin{proof}  Subspace decomposition \eqref{eq:subspacedecomp} holds, definition \eqref{eq:fe:constraintsets} shows $\mathcal{D}^i \subset \cV^i$ and $\mathcal{U}^i \subset \cV^i$, and \eqref{eq:fe:constraintdecomp} implies \eqref{eq:constraintdecomp}.  It remains to find decomposition operators $\Pi_i$ and show \eqref{eq:constraintrestrictionsum}.

For $v \in \mathcal{V}^j$ define the coarsening decomposition (projection) operator
    $$\Pi_j^{j-1} v = \min\{\max\{\iR v, \underline{\chi}^{j-1}\}, \overline{\chi}^{j-1}\}.$$
It is easy to see that $\Pi_j^{j-1}:\mathcal{U}^j \to \mathcal{U}^{j-1}$ because $\iR v \in \mathcal{V}^{j-1}$ and $\underline{\chi}^{j-1} \le \Pi_j^{j-1} v \le \overline{\chi}^{j-1}$.  It also holds that $I-\Pi_j^{j-1}:\mathcal{U}^j \to \mathcal{D}^{j}$ because
    $$v - \Pi_j^{j-1} v FIXME$$
\end{proof}
