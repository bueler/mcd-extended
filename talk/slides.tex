...
\end{Verbatim}
    \begin{itemize}
    \item[$\circ$] CG-GMG V-cycles linear solver applies to inactive variables
    \end{itemize}
\item the outer Newton iteration must converge on the active set \alert{before} multigrid can provide effective preconditioning
    \begin{itemize}
    \item[$\circ$] thus growing Newton (SNES) iterations!
    \end{itemize}
\end{itemize}
...
\section{multigrid approaches for \emph{nonlocal} VIs?}
