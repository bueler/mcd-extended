...
\begin{itemize}
\item see paper (Bueler \& Farrell 2023) for generalization to upper and lower obstacles:
    $$\mathcal{K}^J = \{\underline{\psi}^J \le v^J \le \overline{\psi}^J\}$$
\item see paper for stopping criteria
    \begin{itemize}
    \item[$\circ$] need to evaluate whether CP/KKT conditions are satisfied
    \end{itemize}
\item see paper for FASCD FMG cycle
\end{itemize}
\end{frame}
...
\item suppose it moves by combination of diffusion, advection (wind $\bm{X}(x)$), and source ($\phi(x)$):
    $$-\eps \grad^2 u + \bm{X}\cdot \grad u = \phi$$
\item two active sets (\emph{pictured in $d=2$ case}):
...
\scriptsize
\begin{itemize}
\item<2> compare Klee-Minty cube?
\end{itemize}
\end{frame}
