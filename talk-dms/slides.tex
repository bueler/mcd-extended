...
\title[Fast solvers for PDEs subject to inequalities]{Fast solvers for partial differential equations \\ subject to inequalities}
...
\titlegraphic{\vspace{-8mm} \includegraphics[width=0.25\textwidth]{../talk-oxford/images/uafbw.png} \hfill \begin{minipage}{0.16\textwidth}
...
\item $\mathcal{K}$ is a closed and convex subset of a Sobolev space:
...
\end{align*}
   ...
   \begin{itemize}
   \item[$\circ$] this is not acceptable \dots
   \end{itemize}
\end{itemize}
}
\only<2>{
...
\only<3>{
\begin{itemize}
\item FIXME i.e.~constrained optimization over a convex \emph{admissible set}
	$$\mathcal{K} = \left\{v \in H^1(\Omega) \,:\, v\big|_{\partial \Omega} = g \text{ and } v \ge \psi\right\}$$
...
\subsection{nonlinear multigrid = full approximation scheme (FAS)}
...
\includegraphics[height=0.15\textheight]{../talk-oxford/images/mg-grids.png}

\bigskip

\only<1>{
\hfill \includegraphics[height=0.15\textheight]{../talk-oxford/images/mg-vcycle.png}
}
\only<2>{\hfill \includegraphics[height=0.15\textheight]{../talk-oxford/images/mg-fcycle.png} \quad \begin{tabular}{l} F-cycle = \\ nested iteration \end{tabular}
}
...
\medskip
\begin{center}
\includegraphics[width=0.8\textwidth]{../talk-oxford/images/fascd-firedrake-dome.png}
\end{center}
\end{frame}
