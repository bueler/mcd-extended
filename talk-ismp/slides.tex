...
\begin{frame}{the meanings of ``fast solver''}
...
\end{block}
    \begin{itemize}
    \item[$\circ$] measure work in flops or run-time
    \item[$\circ$] constant iterations of $O(N)$ method $\implies$ optimal
    \end{itemize}
\end{itemize}

\begin{itemize}
\item multigrid people get greedy:
...
\subsection{results}  % FIXME lead with results
...
% FIXME move earlier \section{results}

\begin{frame}{classical obstacle problem by FASCD}

\only<1>{\includegraphics[width=0.75\textwidth]{../talk-dms/figs/ballitersV.png}}\only<2>{\includegraphics[width=0.75\textwidth]{../talk-dms/figs/ballitersVlog.png}}\only<3>{\includegraphics[width=0.75\textwidth]{../talk-dms/figs/ballitersVF.png}}\only<4>{\includegraphics[width=0.75\textwidth]{../talk-dms/figs/bothitersVF.png}} \hfill
